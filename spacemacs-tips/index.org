...
* Introduction and motivation
  As I myself prefer to work on other things than reading documentation about
  how to use certain tools, I will keep this introduction very short.
...
*IMPORTANT COMMENTS:* 
- In info mode most of the evil-bindings are not well
implemented. Therefore it might be handy to read the Emacs documentation about
how to navigate the info manuals.

- The describe function is extremely handy. You should study all describe
  options, (especially: bindings, function, key, mode, variable).
Although the describe function and variable functionalities might get useful
only when writing or studying lisp, the other options are really handy for quick
help lookups.

- With the /binding/ option you can quickly lookup key-bindings by fuzzy search on
  function name (e.g. 'upcase', 'comment', 'jump', 'delete').
- With the /key/ option you can quickly find the function name bound to a
  key-sequence by searching on key sequence (e.g. =SPC l=, or =g g=).
- With the /mode/ option you can quickly get information about the current major
  mode (e.g. special functions and bindings for the python mode)
