...
** deep-search
 ...
 directory. Instead use grep there =SPC s g=
** org notes
...
** quick navigation within files
 To quickly navigate inside a file use =SPC j i=. Additionally one could choose to [[info:emacs#Specifying File
  Variables][set a local variable]] to start with the code folded using ~HideShow~. E.g. replace
